...
namespace Kythr.Tests.Publishers
{
    [TestFixture]
    [Category("Unit")]
    public class GoogleChatPublisherShould
    {
        private IHealthChecksBuilder _healthChecksBuilder;
        private GoogleChatTransportSettings _settings;
        ...
        public void Setup()
        {
            ...
            _settings = new GoogleChatTransportSettings
            {
                Name = "GoogleChatTest",
        ...
        {
            var healthCheck = CreateHealthCheck("gchat_test", "GoogleChatTest", AlertTransportMethod.GoogleChat);
            var publisher = new GoogleChatAlertingPublisher(_healthChecksBuilder, healthCheck, _settings);
        ...
        {
            var invalidSettings = new GoogleChatTransportSettings { Name = "Invalid", WebhookUrl = null };
            var healthCheck = CreateHealthCheck("gchat_test", "Invalid", AlertTransportMethod.GoogleChat);

            Assert.Throws<GoogleChatValidationError>(() =>
            {
                var publisher = new GoogleChatAlertingPublisher(_healthChecksBuilder, healthCheck, invalidSettings);
                publisher.SetUp();
            });
        }
        ...
        {
            var invalidSettings = new GoogleChatTransportSettings { Name = "Invalid", WebhookUrl = "" };
            var healthCheck = CreateHealthCheck("gchat_test", "Invalid", AlertTransportMethod.GoogleChat);
        ...
        {
            var healthCheck = CreateHealthCheck("gchat_pub", "GoogleChatTest", AlertTransportMethod.GoogleChat);
            var publisher = new GoogleChatAlertingPublisher(_healthChecksBuilder, healthCheck, _settings);
            var report = CreateHealthReport("gchat_pub", HealthStatus.Unhealthy, "Service down");
        ...
        {
            var healthCheck = CreateHealthCheck("gchat_fail", "GoogleChatTest", AlertTransportMethod.GoogleChat);
            var publisher = new GoogleChatAlertingPublisher(_healthChecksBuilder, healthCheck, _settings);
            var report = CreateHealthReport("gchat_fail", HealthStatus.Unhealthy, "Down");
