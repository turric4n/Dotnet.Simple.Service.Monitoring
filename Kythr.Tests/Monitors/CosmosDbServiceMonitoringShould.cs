using System;
using FluentAssertions;
using Microsoft.Extensions.DependencyInjection;
using NUnit.Framework;
using Kythr.Library.Models;
using Kythr.Library.Monitoring.Exceptions;
using Kythr.Library.Monitoring.Implementations;

namespace Kythr.Tests.Monitors
{
    [TestFixture]
    [Category("Unit")]
    public class CosmosDbServiceMonitoringShould
    {
        private IHealthChecksBuilder _healthChecksBuilder;

        [SetUp]
        public void Setup()
        {
            var mock = new Moq.Mock<IHealthChecksBuilder>();
            mock.Setup(m => m.Services).Returns(new ServiceCollection());
            _healthChecksBuilder = mock.Object;
        }

        [Test]
        public void Given_Valid_CosmosDb_Monitoring_Settings()
        {
            var healthCheck = new ServiceHealthCheck
            {
                Name = "cosmosdb_test",
                ServiceType = ServiceType.CosmosDb,
                ConnectionString = "AccountEndpoint=https://myaccount.documents.azure.com:443/;AccountKey=abc123==;",
                HealthCheckConditions = new HealthCheckConditions
                {
                    CosmosDbBehaviour = new CosmosDbBehaviour
                    {
                        DatabaseId = "mydb",
                        ContainerId = "mycontainer",
                        TimeOutMs = 5000
                    }
                }
            };

            var monitoring = new CosmosDbServiceMonitoring(_healthChecksBuilder, healthCheck);

            Action act = () => monitoring.SetUp();
            act.Should().NotThrow();
        }

        [Test]
        public void Throw_When_ConnectionString_Is_Null()
        {
            var healthCheck = new ServiceHealthCheck
            {
                Name = "cosmosdb_test",
                ServiceType = ServiceType.CosmosDb,
                ConnectionString = null,
                HealthCheckConditions = new HealthCheckConditions
                {
                    CosmosDbBehaviour = new CosmosDbBehaviour
                    {
                        DatabaseId = "mydb",
                        ContainerId = "mycontainer"
                    }
                }
            };

            var monitoring = new CosmosDbServiceMonitoring(_healthChecksBuilder, healthCheck);

            Action act = () => monitoring.SetUp();
            act.Should().Throw<InvalidConnectionStringException>();
        }

        [Test]
        public void Throw_When_ConnectionString_Is_Empty()
        {
            var healthCheck = new ServiceHealthCheck
            {
                Name = "cosmosdb_test",
                ServiceType = ServiceType.CosmosDb,
                ConnectionString = "",
                HealthCheckConditions = new HealthCheckConditions
                {
                    CosmosDbBehaviour = new CosmosDbBehaviour
                    {
                        DatabaseId = "mydb"
                    }
                }
            };

            var monitoring = new CosmosDbServiceMonitoring(_healthChecksBuilder, healthCheck);

            Action act = () => monitoring.SetUp();
            act.Should().Throw<InvalidConnectionStringException>();
        }

        [Test]
        public void Given_Valid_CosmosDb_Without_ContainerId()
        {
            var healthCheck = new ServiceHealthCheck
            {
                Name = "cosmosdb_no_container",
                ServiceType = ServiceType.CosmosDb,
                ConnectionString = "AccountEndpoint=https://myaccount.documents.azure.com:443/;AccountKey=abc123==;",
                HealthCheckConditions = new HealthCheckConditions
                {
                    CosmosDbBehaviour = new CosmosDbBehaviour
                    {
                        DatabaseId = "mydb",
                        TimeOutMs = 3000
                    }
                }
            };

            var monitoring = new CosmosDbServiceMonitoring(_healthChecksBuilder, healthCheck);

            Action act = () => monitoring.SetUp();
            act.Should().NotThrow();
        }
    }
}
